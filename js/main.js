...
$(document).ready(function () {
    ...
    function addScroll(element) {
        ...
        let top = 0; // отпустили мышку - сохраняем положение скролла
        let drag = false;
        let coorY = 0; // нажали мышку - сохраняем координаты.
    ...
    // Функционал кнопок для скролла
    let flag = false;
    let interval;
    const arrows = $('.img__item-arrow');
    arrows.each(function(index, arrow) {
      const item = $(arrow).closest('.portfolio__item');
    
      $(arrow).mousedown(function(e) {
        flag = true;
        const img = $(e.target).closest('.port-test').find('.img__item');
        const scrollSpeed = parseInt($(e.target).data('scrollspeed'));

        interval = setInterval(function() {
          if (!flag) return;
          img.scrollTop(scrollSpeed + img.scrollTop());
        }, 20);
      });
    ...
    $('.portfolio__item .img__item').each(function() {
        addScroll(this);
    });        
});
